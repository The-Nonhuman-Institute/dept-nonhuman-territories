...
def run_shift(dry_run: bool = False) -> int:
    ...
    try:
        ...
        if not any(c["census_density"] > 0 for c in world["cells"]):
            ...
            #
            # BASIN-02: a Generator position is a fraction of the GRADIENT, so
            # it selects a depth, not a flat cell index. Both seeds go in the
            # middle column, which is the only lateral choice that treats the
            # field symmetrically and prefers no direction. The depths chosen
            # are the same depths BASIN-01 seeds: 0.85 and 0.15 of the way to
            # the data-stream.
            middle = life.FIELD_WIDTH // 2
            life.seed_census(world, life.cell_index(
                int(config.GENERATOR_A_POSITION * (life.FIELD_DEPTH - 1)), middle),
                generator_a.SUBSTRATE)
            life.seed_census(world, life.cell_index(
                int(config.GENERATOR_B_POSITION * (life.FIELD_DEPTH - 1)), middle),
