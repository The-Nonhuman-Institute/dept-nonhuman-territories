...
_APPEND_ONLY_FILES = frozenset(
    (config.SPECIMEN_LOG, config.ANOMALY_LOG, config.SHIFT_LOG, config.FIELD_LOG)
)
...
class Transaction(object):
    ...
    def __init__(self, shift_number: int) -> None:
        ...
        self._field_records: List[Dict[str, Any]] = []
    ...
    def append_field_snapshot(self, record: Dict[str, Any]) -> None:
        """The untouched field for this shift (BASIN-02).

        Append-only, like every other log here. This is the raw material the
        aggregate census figures are reduced from; keeping it means the
        reduction is inspectable after the fact instead of being a number
        nobody can get behind.
        """
        self._require_open()
        self._field_records.append(self._stamp(record))
    ...
    def commit(self, mark_shift: bool = True) -> Dict[str, int]:
        ...
        written = {
            ...
            "field_records": _append_lines_atomic(
                config.FIELD_LOG, self._field_records
            ),
        }
...
class _SummaryWriter(object):
    """The Keeper's view: the continuity summary, and the field record."""
    ...
    def append_field_snapshot(self, record: Dict[str, Any]) -> None:
        self._txn.append_field_snapshot(record)
