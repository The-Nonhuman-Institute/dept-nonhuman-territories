...
def run(
    ...
    for record in batch:
        ...
        if decision == "new_category":
            # The Namer sometimes declares a category new that it has already
            # coined — 4 of 15 coinage events on BASIN-03, 1 of 3 on BASIN-04.
            # Its claim is kept exactly as made, on the specimen record, and is
            # never overruled. What is split here is only the COUNT, because a
            # shift log that reports the same category coined twice inflates
            # every downstream figure that reads it.
            #
            # A re-declaration is also worth seeing rather than silently
            # merging away: it is the Namer losing track of its own taxonomy,
            # which is an observation about the Namer.
            if category in (stats or {}):
                outcome.setdefault("recoined_categories", []).append(category)
            else:
