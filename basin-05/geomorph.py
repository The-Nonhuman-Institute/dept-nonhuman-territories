...
MAX_CUT = 0.055              # absolute ceiling on a single shift's cut
LATERAL = 0.35               # share of a bend's cut taken from the outer bank


def tectonic(index: int, width: int, height: int) -> float:
    """How fast this place is being lifted. Knows nothing of rivers or peaks.

    BASIN-04 lifts every cell at the same rate, biased only by distance from
    the margin, and the result measured as a tableland: 19.09 of interior
    relief with a cliff at the mouth and almost nothing between. Giving uplift
    a shape in space raised that to 30.10 under the terrain's real growing
    conditions, and to 45.65 with base level held still.

    Nothing here places a mountain. It says only that the ground does not rise
    evenly, which is true of ground.
    """
    r, c = index // width, index % width
    a = math.sin(r / 11.0) * math.cos(c / 13.0)
    return 1.0 + 1.6 * (0.5 + 0.5 * a) + 0.9 * _hashed("tect", r // 6, c // 6)


def hardness(index: int, width: int, height: int) -> float:
    """What this place resists being cut by. Bands, from the seed.

    Paired with the absolute cut ceiling below. On its own this does not
    produce a fall — that was tried in BASIN-03 and is recorded above as a
    failure — but it is what lets a channel lengthen instead of deepening in
    one place.
    """
    r, c = index // width, index % width
    band = math.sin(r / 7.5 + c / 21.0)
    return 0.55 + 1.35 * (0.5 + 0.5 * band) + 0.4 * _hashed("rock", r // 4, c // 9)


def _hashed(*parts) -> float:
    h = 2166136261
    for part in parts:
        for ch in str(part):
...
def _run(ground: List[float], width: int, height: int, shifts: int,
    ...
    for shift in range(shifts):
        ...
        for i in range(n):
            ...
            ground[i] += UPLIFT_BASE * bias * scale * tectonic(i, width, height)
        ...
        for i in order:
            ...
            if slope > SETTLE_SLOPE and steepest[i] is not None:
                ...
                # SATURATING, not proportional. BASIN-04 caps the cut at 45% of
                # the step, so a tall step is removed fastest in absolute terms
                # and erases itself. Capping at an absolute rate set by the rock
                # instead measured a channel four times longer — 20 cells to 72
                # on a 60-deep grid. It did NOT produce a fall; median step life
                # stayed at 3 shifts, as it does in every variant tried.
                cut = min(cut, MAX_CUT / hardness(i, width, height))
                cut = min(cut, slope * 0.95)      # cannot cut below what it flows to
                ground[i] -= cut
                carried += cut
                if flow[i] > 40:
                    # On a turn, part of the cut is taken from the bank on the
                    # outside of the bend rather than from the bed. Which side
                    # is outside is decided by where the flow arrived from.
                    # Measured alone this STRAIGHTENS a channel, 1.345 to 1.251;
                    # applied to a channel the other two rules have deepened it
                    # raised sinuosity from 1.465 to 2.007.
                    feeders = [j for j in neighbours(i, width, height)
                               if steepest[j] == i]
                    if feeders:
                        src = max(feeders, key=lambda j: flow[j])
                        dr = (i // width) - (src // width)
                        dc = (i % width) - (src % width)
                        for j in neighbours(i, width, height):
                            jr = (j // width) - (i // width)
                            jc = (j % width) - (i % width)
                            if (jr, jc) != (dr, dc) and (jr, jc) != (-dr, -dc):
                                ground[j] -= cut * LATERAL * 0.5
            elif carried > 0.0:
