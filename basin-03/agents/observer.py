...
def observation_text(record: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "position across the field: column %d of %d (carries no light)"
        % (record["position_across_field"], life.FIELD_WIDTH),
        # Elevation entered the physics at shift 141 and was not added here
        # until shift 181. For forty shifts the Namer classified specimens in
        # a terrain where residue varied thirty-threefold by height, while
        # being told nothing about height. That was an omission in what was
        # reported, not a finding about the terrain.
        #
        # Stated as measurement and mechanism, the same way the light gradient
        # is stated. No landform is named and no place is called good or poor:
        # what a high or low position amounts to is the Namer's to decide.
        "elevation: %.2f (0.00 lowest in the field, 1.00 highest; "
        "residue runs downhill, so what falls high drains away and what falls "
        "low collects)" % record["elevation"],
...
def census_observation(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    BASIN-02 note: this terrain is a field of 315 cells, not a line of 21. The
    per-cell density list BASIN-01 sends would be fifteen times the size and is
    replaced by the rollup in life.field_rollup(), which states its own
    arithmetic. Nothing else about what is observed differs.
    """
    ...
    return {
        ...
        # BASIN-02: 315 cells cannot ship as a list, so physics.md Section 8's
        # aggregate tier reduces the field. density_by_depth is the same shape
        # and meaning as BASIN-01's per-cell list — that list IS the depth
        # profile of a one-column field. density_by_column is what a line does
        # not have. Every figure is defined in life.field_rollup() and the raw
        # grid it was computed from is in state/field_log.jsonl.
        "field": summary["field"],
        "population_spread": life.population_spread(state),
    }
...
def select_for_observation(state: Dict[str, Any], shift: int,
    ...
    # BASIN-03 is large enough that five observations drawn purely by age would
    # all come from wherever the population happens to be oldest — one corner of
    # a terrain that may be thousands of cells across. So the sample rotates
    # through REGIONS by shift number, deterministically, and applies the same
    # mechanical ordering inside whichever region comes up.
    #
    # This is still not a way to show the Namer the interesting ones. Which
    # region is looked at is decided by arithmetic on the shift number before
    # anything is measured, and the ordering within it is unchanged: never seen
    # first, then longest present, then most descendants, ties on identifier.
    regions = getattr(life, "REGION_COUNT", 0)
    if regions > 1 and individuals:
        band = shift % regions
        in_band = [b for b in individuals
                   if life.region_of(int(b.get("cell", 0))) == band]
        if in_band:
            individuals = in_band
